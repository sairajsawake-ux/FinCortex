...
fun ActivityHubScreen(navController: NavController) {
    ...
    Column(
        ...
        Row(
            ...
            Text(text = "Activity Hub", color = MaterialTheme.colorScheme.onBackground, fontSize = 20.sp, fontWeight = FontWeight.Bold)
            TextButton(onClick = { /* TODO */ }) {
                Text("CLEAR ALL", color = MaterialTheme.colorScheme.tertiary)
            }
        }
        Text(text = "${notifications.size} RECENT NOTIFICATIONS", color = MaterialTheme.colorScheme.onBackground.copy(alpha = 0.7f), fontSize = 12.sp, modifier = Modifier.padding(start = 16.dp))
    ...
    Card(
        ...
        Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(text = notification.title, color = MaterialTheme.colorScheme.onSurface, fontWeight = FontWeight.Bold)
                Text(text = notification.description, color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.8f), fontSize = 14.sp)
                if (notification.amount != null) {
                    Text(text = notification.amount, color = MaterialTheme.colorScheme.tertiary, fontWeight = FontWeight.SemiBold, modifier = Modifier.padding(top = 4.dp))
                }
            }
            Text(text = notification.time, color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.6f), fontSize = 12.sp)
        }
    }
}
